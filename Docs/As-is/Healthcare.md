...
4.  `auth-service` отправляет SQL-запрос к `auth-db`: выборка из таблицы `users`, где `email` совпадает со значением переменной `email`, которую передал пользователь в JSON-объекте.
5.  `auth-db` в таблице `users` находит запись и возвращает данные.
...
| **3** | Регистрация пациента и создание счета | **MVP** | [patient-service](Services%2Fpatient-service.md)  <br/> [billing-service](Services%2Fbilling-service.md)  <br/> [analytics-service (async consumer)](Services%2Fanalytics-service.md) <br/> [kafka](Services%2Fkafka.md) | [patient_db](DB%2Fpatient_db.md)                | [POST /patients](API%2FPOST.patients.md) <br/> [CreateBillingAccount](API%2FCreateBillingAccount.md) <br/> [PatienEventProducer](API%2FPatienEventProducer.md) <br/> [PatientEventConsumer](API%2FPatientEventConsumer.md)                                             |
